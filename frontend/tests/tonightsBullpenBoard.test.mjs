...
test('team board adopts the v2 Answer Block, Active Bullpen, Recent Usage, and Rest Status', () => {
  ...
  assert.ok(tonightsBullpenBoardSource.includes('getTeamBoardCore(selectedTeam, options)'))
  assert.ok(tonightsBullpenBoardSource.includes('getTeamBoardDetails(selectedTeam, coreIdentity, options)'))
...
test('view helpers group, total, and detect stale freshness deterministically', () => {
  ...
  assert.equal(view.getBullpenStressView(populatedBoard.stress).summary, 'Cleanly available options are limited right now.')
  assert.equal(view.getBullpenStressView(populatedBoard.stress).summary.includes('than usual'), false)
