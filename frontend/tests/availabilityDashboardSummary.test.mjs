...
test('formats current-mode availability summary distributions', () => {
  ...
  assert.equal(view.operationalSummary, 'Most current bullpen arms are in the Monitor lane.')
...
test('renders compact availability summary with secondary evidence collapsed', () => {
  ...
  assert.ok(htmlIncludes(collapsedHtml, 'Bullpen availability mix. Most current bullpen arms are in the Monitor lane.'))
  assert.ok(htmlIncludes(collapsedHtml, 'Monitor: 704 (100%)'))
  assert.ok(htmlIncludes(collapsedHtml, 'Most current bullpen arms are in the Monitor lane.'))
...
test('renders scored pitcher inventory without current availability labeling', () => {
  ...
  assert.equal(view.operationalSummary, 'Most stored pitcher workload reads are in the Monitor lane.')
