...
test('Roster Context reuses v2 authority and replaces the legacy Team Board banner', async () => {
  const html = renderTransactions({ read })
  ...
  assert.ok(html.includes('Roster Context'))
...
test('production reuses one v2 request, preserves placement, and does not start What Changed', async () => {
  ...
  assert.ok(boardSource.indexOf('<TeamBoardRecentTransactions') < boardSource.indexOf('<StoryCard'))
