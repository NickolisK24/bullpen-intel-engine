...
| 12 | Phase 0K - Methodology, Limitations, and Data Trust Rewrite | Complete | Completed across two public-surface branches. The Methodology page was rewritten public-first (feat/methodology-public-first-rewrite, PR #503, commit 7a1bf39): evidence examined, how an arm read and a team read are formed, one fixed illustrative worked example, freshness and publication safeguards, and limitations. The Data & Trust page was rewritten reader-first (feat/data-trust-reader-first-rewrite, PR #504, commit d811e0c) to lead with the current public-data answer from served dashboard freshness, explain freshness and coverage (data-through vs updated vs last-checked), and present the retrospective next-day usage check with unknown-versus-zero-honest formatting; the scored-pitcher inventory diagnostic was removed and Methodology and How to Read are linked. Presentation-layer only: no availability, usage-check, threshold, sync, snapshot, or API changes. |
| 13 | Phase 0L - Analytics Event Alignment | Complete | Aligned the existing privacy-bounded, route-based traffic measurement with the evidence-first product (feat/analytics-evidence-alignment) without changing any baseball intelligence, classification, evidence, freshness, page hierarchy, route, or public claim. Added the bounded `since_yesterday` entry source on both sides of the wire — frontend canonical source list and backend entry-source allowlist, kept in parity by a dedicated test — so Since Yesterday trusted-change links carry provenance and are reported separately as Since Yesterday Evidence Opens. Added one consolidated internal "Evidence & Trust Use" reporting section that reuses the same canonical page-view rows: Team Read, Recent Bullpen Work, Pitcher Lane, Reliever Detail, Comparison Read, and Comparison Evidence views; Reliever Finder views (stored key all_pitchers kept for historical continuity); Methodology and Data & Trust views; Since Yesterday Evidence Opens; Sessions With Deeper Evidence; and Evidence Depth %. Every definition states that a page view is an opening, not proof that the visitor read, understood, or trusted the destination, and the section carries no rankings or scores. The internal Traffic Intelligence dashboard now uses current product names (Reliever Finder) and renders the new section, with an em dash for unknown and zero for an explicit zero. Route dedup and evidence-target classification were verified unchanged; privacy remains bounded (no names, emails, search text, story copy, feedback text, full referrer URLs, reason codes, or raw freshness). Within-page interactions — last-outing and pitch-trend expanders, stale/partial disclosure toggles, and feedback interactions — remain not separately measurable by the route-based system by design and were left uninstrumented to preserve privacy; no analytics UI or fabricated metrics were added. Additive fields only, with frontend and backend allowlists in parity. |
