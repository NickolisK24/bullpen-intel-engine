...
| UX-001 (#590) | **Production validation failed; correction merged, revalidation pending** | The canonical Team State contract is live, but production published Vulnerable for every supported team: readiness distributions were built from every active-flagged pitcher while trust coverage used the canonical active bullpen, so one starter or injured-list arm forced the whole club stressed. Population corrected; the Fresh/Stretched/Vulnerable/fail-closed production evidence must be captured against the corrected build |
...
| Production validation failed; correction merged, revalidation pending | UX-001 (#590) | Fresh, Stretched, Vulnerable, and fail-closed production evidence, captured against the corrected population |
