...
D-054 was added after this version's reconciliation, by UX-003 (Product Experience Foundation Migration). It grants a presentation-only authority over an enumerated set of protected frontend files. It adds no baseball semantics and changes neither D-051 nor D-052.

D-055 was added after this version's reconciliation, by UX-003 Pass 2 (Team Board answer zone). It extends the same presentation-only authority to two further enumerated files. It adds no baseball semantics and changes neither D-051 nor D-052.

D-056 was added after this version's reconciliation, by UX-003 Pass 3 (approved token alignment and Team Board convergence). It extends the same presentation-only authority to five further enumerated runtime files and two existing contract tests. It adds no baseball semantics and changes neither D-051 nor D-052.
...
| D-054 | Aug 13, 2026 | The approved Product Experience Foundation migration (UX-003) may modify an enumerated, exact-path set of protected frontend files, recorded in `backend/tests/product_experience_files.py`. The authority is presentation and migration only: the visual system, shared presentation primitives, typography, spacing, geometry, responsive behaviour, accessibility contrast, canonical public navigation labels, the Today Daily Edition hierarchy, freshness presentation, evidence presentation, and presentation-only disambiguation of vocabulary canon already defines. It authorizes no Team State derivation, bullpen tier, availability computation, threshold, prediction, recommendation, ranking, fatigue score, invented backend claim, fabricated baseball fact, Today lead-story claim family, or backend API semantic change. The backend remains the owner of baseball meaning; public Team State remains exactly Fresh, Stretched, and Vulnerable. No wildcard or directory exemption is granted, and migrating any further protected surface requires a new or amended governed decision reviewed on its own evidence. No baseball semantics move into presentation; D-051 and D-052 are unchanged. | Standing presentation boundary; repository implementation only, not production-verified |
| D-055 | Aug 13, 2026 | UX-003 Pass 2 extends the D-054 presentation-only authority to the Team Board answer zone, adding exactly `frontend/src/components/bullpen/BullpenOperatingStateCard.jsx` and `frontend/src/components/bullpen/board/BullpenAvailabilityDistribution.jsx` to the enumerated allowlist. The pass recomposes the Team Board's answer in reading order — team identity, canonical Team State, backend Why, evidence, freshness, limitations — and stops the state reading as corner metadata or tinting the whole panel. It authorizes no Team State derivation, arm-availability derivation, workload scoring, bullpen tier, threshold, prediction, recommendation, ranking, deployment forecast, manager-intent or health inference, backend API semantic change, public claim invention, fallback baseball fact, or new vocabulary family. Team State remains backend-owned and exactly Fresh, Stretched, and Vulnerable; role and arm read remain separate concepts. M-001 Active Bullpen ERA remains reserved and non-public under D-022 and is not introduced. The remaining Team Board regions and every other public surface stay unlisted and require their own amendment. D-051, D-052, and D-054 are unchanged. | Standing presentation boundary; repository implementation only, not production-verified |
| D-056 | Aug 13, 2026 | UX-003 Pass 3 aligns the existing shared token layer with the approved Claude Design HTML — `#0B0F14`, `#10161E`, `#141C26`, `#232C38`, the approved text/accent/state hues, Archivo Narrow, IBM Plex Sans, IBM Plex Mono, and square geometry — and extends the exact-path presentation authority to `frontend/src/components/bullpen/Bullpen.jsx`, `frontend/src/components/bullpen/PitcherSearch.jsx`, `frontend/src/components/bullpen/TeamReliefWorkPanel.jsx`, `frontend/src/components/bullpen/board/BullpenBoardView.jsx`, and `frontend/src/components/bullpen/board/TonightsBullpenBoard.jsx`. The Team Board may now converge its page chrome, team selector and search, Recent Bullpen Work, Current Arm Picture, responsive rows, and narrative disclosures while rendering the same governed payloads. It authorizes no Team State or arm-read derivation, score, ranking, recommendation, prediction, manager-intent or health inference, new claim, sample-data promotion, API change, or Active Bullpen ERA. Role and read remain separate, unknown stays unknown, degraded reads remain visible, and the exact-path guards remain fail-closed. D-051 through D-055 are unchanged. | Standing presentation boundary; repository implementation only, not production-verified |
| D-057 | Aug 13, 2026 | UX-003 Pass 4 converges the shared public presentation on the approved HTML Foundations contract: a 1360px shell with 24px desktop gutters, 1160px reading measure, 44px desktop section rhythm, square geometry, no shadows or decorative depth, the 62px publication masthead, blue-to-gold intelligence rules, indexed evidence receipts, diamond Team State marker, and left-rule non-loading states. The same presentation authority extends exactly to the shared footer; the Dashboard, Stories, Methodology, and Data & Trust route shells; and the Pitcher Detail identity, current-read, recent-work, and evidence presentation in `AvailabilitySummary.jsx`, `PitcherDetail.jsx`, and `RecentWorkPanel.jsx`. The accessibility-safe muted and evidence-index colors remain the canonical production tokens. This pass changes no baseball data, derivation, threshold, state, ranking, prediction, recommendation, freshness authority, publication gate, or API contract. | Standing presentation boundary; repository implementation only, not production-verified |
