...
| D-042 | Aug 4, 2026 | **The first production no-op qualification refused on a missing durable work item; candidate selection now comes from a read-only audit rather than operator intuition.** Run `30862655470` dispatched the manual no-op write qualification for game 824488 and returned FAILED with one reason: `target_work_item_missing`. **Every safety property held** — authorization passed, the writer guard was acquired and released, the write phase was never entered, no baseball-data or lane-bookkeeping effect was produced, no transaction mutation occurred, and the evidence artifact scanned clean and uploaded before the gate failed as designed. **What it proved is the finding**: a GameLog row that exists and reconciles is NOT the same fact as a completed durable `game_ingestion_work_item`. The lane creates that row only in a write-capable mode, and it has only ever run in shadow — so no amount of inspecting reconciled statistics can tell an operator whether a game is a valid qualification target. Guessing again would have produced the same refusal on a different game. **Decision: derive the candidate from production evidence, read-only.** A separate manual-only workflow (`workflow_dispatch` only, main-only, owner-only, `baseballos-sync` concurrency, `cancel-in-progress: false`, 20-minute bound) runs a bounded, deterministically ordered scan of completed durable work items — `completed_at DESC, represented_date DESC, mlb_game_pk ASC`, never database default order — and runs the **canonical shadow lane** with exact one-game scope against each. Bounds are enforced twice, in shell and in Python: lookback 1–120 (default 30), candidate limit 1–50 (default 20), eligible target 1–10 (default 5) and never above the candidate limit. **Eligibility requires positive evidence of every condition** — planner executed, finality positively established for exactly the requested game, exact scope, a non-empty plan whose every row is `unchanged`, zero inserts, updates, blocked rows, canonical-outs corrections, statistical corrections, authority reconciliations, appearance-team mutations and identity actions, exactly one non-null source revision for the requested game, and a plan fingerprint. Nothing is eligible because a failure is absent. Each candidate receives exactly one primary classification under a declared precedence: read-only violation, then execution/unproven, then durable-work-item invalidity, then finality/scope, then mutation/identity, then revision/fingerprint, then eligible. **Read-only is proven three ways**: the acquire-only public sync advisory lock (it never touches SyncRun rows), a PostgreSQL `SET TRANSACTION READ ONLY` with a refused `WHERE 1 = 0` write probe reused from the existing production diagnostics, and before/after content fingerprints — full row content and timestamps, not row counts — across `game_logs`, `pitchers`, `game_ingestion_work_items`, `postgame_processed_games`, `scheduled_games`, and `sync_failures` (the dead-letter target). The read-only transaction is re-asserted after every candidate because the shadow lane ends with a rollback. AST and structural tests prove the service contains no session write API, never commits, reaches the lane in shadow only, and contains exactly one SQL mutation verb: the bounded refused probe. **Zero eligible candidates is `COMPLETE_NO_ELIGIBLE_CANDIDATE` and exits 0** — a completed audit, not a failure. The audit creates no work item, dispatches no workflow, and weakens no existing contract; the suggested candidate is the first eligible game under the declared ordering and is explicitly informational. **Two database guarantees were discovered and recorded rather than assumed**: `uq_game_ingestion_work_items_game_pk` makes duplicate work-item identity impossible, and `ck_game_ingestion_work_items_completion_proof` makes a completed row without a completion timestamp and an exactly reconciled expectation impossible — so both corresponding guards are defensive redundancy, and the classifications cannot fire while those constraints stand. **A second evidence defect was corrected in the same package**: the failed artifact reported `finality_proven_by_planner: true` although execution stopped at the work-item precondition before the planner ran, because that field was derived from the ABSENCE of a not-plannable failure. Explicit execution-state fields now record what actually ran — `work_item_precondition_checked`, `work_item_precondition_passed`, `planner_phase_entered`, `finality_check_executed`, `finality_proven_by_planner` — with finality true only when the planner positively planned the exact requested game, null when the check never executed, and rendered as `not executed` in the summary. Run `30862655470` has a regression fixture. **If no candidate exists**, the next separately reviewed decision is between a governed exact-scope lane-ledger initialization package and allowing the first qualification to create exactly one work item under a separately governed creation contract; neither is authorized here. No migration was added; the Alembic head remains `c7f1b408d93a`. Daily and postgame remain `shadow`, backfill remains `off`, publication authority is unchanged, the legacy writer remains production-authoritative, and automated write and authoritative modes remain unapproved | **Review correction (same package):** six runtime/evidence blockers were closed before merge. (1) Eligibility now requires the COMPLETE shared plan-governance result — unknown actions zero, prohibited actions zero, unchanged count equal to planned row count, no non-zero mutation counter, and `all_rows_already_matching` exactly true — plus a final positive predicate; a candidate reaching the end with no recognised reason but an unmet condition fails closed rather than inheriting eligibility from silence, and an unrecognised future action type fails closed. (2) Eligibility now requires the lane's exact successful-completion status `complete`; every other known status maps to a named refusal and anything unrecognised fails closed, so unchanged-looking rows cannot rescue a refused or partial run. (3) The qualification's real execution order was corrected: it previously ran the shadow lane BEFORE reading the durable work item, so run 30862655470's refusal spent an MLB request and a full planning pass on an outcome already decided, and the evidence recorded a planner phase it never depended on. The work-item precondition is now read first and a missing item refuses without calling the lane or MLB; the regression test drives the real orchestration rather than a helper fixture. (4) The read-only probe is reported honestly: the audit does issue exactly one bounded `UPDATE ... WHERE 1 = 0` proof statement, so `writes_attempted_by_audit = 0` was replaced by explicit probe fields (attempted, count, statement class, bounded-to-zero-rows, refused) alongside `durable_write_attempts = 0`, with the statement text never reproduced. (5) The bounded stop reason no longer reports `candidate_limit_reached` for a pool smaller than the configured limit; limit, pool size, and selection size now decide it. (6) `candidate_pool_size`, `candidates_selected`, `candidates_evaluated`, `eligible_stop_position`, and `configured_candidate_limit` are reported separately so early stopping cannot inflate the evaluated count. Python numeric parsing was also tightened to digits-only with no leading plus, matching the workflow gate exactly | Adopted |
| D-043 | Aug 4, 2026 | **The failed postgame publication cycle is diagnosed by a manual, exact-scope, read-only incident audit; nothing is repaired, and the shadow lane is explicitly NOT the failure.** Scheduled run `30873422601` (branch `main`, head `9f9f640799af973f0a39cdafb1db83fba473b10c`, cycle `postgame`, slate 2026-08-03) exited 1 while every component inside it reported success. **The correct classification is recorded as governance, not as opinion**: game-driven shadow execution PASS; legacy postgame baseball ingestion completed successfully; appearance-ledger publication qualification FAILED; dashboard publication qualification FAILED; complete postgame activation cycle FAILED. A perfect shadow reconciliation result does not prove the publication system correct, and a publication failure does not invalidate the reconciliation engine. **The immutable incident evidence is preserved in code**: the shadow lane planned/fetched/completed 35 games with 0 failed, 295 rows expected and 295 unchanged, zero projected inserts/updates/blocked rows, zero canonical-outs and statistical corrections, zero identity and appearance-team mutations, zero dead letters, zero baseball rows written, writes disabled, publication non-authoritative, exact scope, no budget stop, 13.794s against a 150.0s allocation, status `complete`; legacy postgame reported success with 35 completed games found, 30 already processed, 5 newly processed (823431, 823520, 823757, 824160, 824647), 40 new GameLog rows, 0 corrected, 10 completed-game contexts, 10/10 progressive team publications; the preflight mapped 822867, 824324, and 825095 to `other` and the other five to `final`; the ledger over 2026-07-25..2026-08-03 expected 133 games and represented 132 with 1110 expected and 1110 stored appearances, 9 mismatches across 9 players, missing game_pk 822867, a 5-of-6 hole on 2026-08-03, publish eligible NO for `final_games_without_appearance_rows`; snapshot 344 stayed pending (unpublished, `published_at` null, data through 2026-08-03, availability reference 2026-08-04, sync run 596) while snapshot 343 (data through 2026-08-02) kept serving, with publication proof reporting `candidate_snapshot_not_ready`, `candidate_snapshot_not_published`, `candidate_snapshot_published_at_missing`, `dashboard_snapshot_slate_coverage_incomplete`, and `candidate_not_selected_for_serving`; and completeness reported horizon 7, 102 expected final games, 42 completed, 60 unresolved, 377 critical appearance rows expected and 377 reconciled, zero finality conflicts, zero missing schedule authority, zero terminal failures. **The mechanism read from code — and stated as a hypothesis for the audit to prove or refute, not as a finding** — is that `services.appearance_ledger` counts a game when ANY stored `scheduled_games` row carries `status_state = final` inside the window, while `services.game_ingestion_planner` plans a game only when EVERY stored row agrees, and `other` is where the finality authority places cancelled, live, abstract-final-without-final-status, and unknown status alike; a game can therefore be inside ledger membership and outside planning scope at once. **Decision: reconstruct the cycle read-only, exactly once, from four separately labelled evidence sources** — immutable incident artifacts, current database state, current official MLB source, and derived inference — and answer eight questions individually, where one unanswered question makes the whole audit UNPROVEN and no question is ever answered by the absence of a failure. **Evidence is addressed by exact run id and exact artifact name** with known artifact ids, retentions, and sha256 digests treated as expectations to VERIFY: an unexposed digest is UNVERIFIED and does not fail, a present-but-different digest is FAILED, and run id, head SHA, cycle kind, slate date, runner exit code, and disputed game id are read from inside the documents rather than trusted from filenames. The shadow activation and ledger artifacts are required; the 7-day handoff artifact is optional because its evidence is duplicated, and its absence is REPORTED rather than inferred away. **Read-only is proven three ways** — the acquire-only public sync advisory lock, a PostgreSQL read-only transaction with a bounded refused write probe, and before/after SCOPED content fingerprints over full governed row content and timestamps across six scopes (exact game 822867, slate 2026-08-03, the ledger window, snapshots 343 and 344, sync run 596, and the unresolved-game set); a changed scope is FAILED and an uncomputable required fingerprint is UNPROVEN. Correction provenance was found to be COLUMNAR rather than a table — it lives on `game_logs`, `team_game_pitching_splits`, `game_play_by_play_events`, and `game_ingestion_work_items` — so full-row digests already cover it and no new table was invented to hold it. **The upstream source is bounded** to 8 schedule, 10 exact-game, 10 box-score, and 20 total calls, reserved before issue with attempts, successes, failures, retries, refusals, latency, and remaining budget reported separately; exhaustion is UNPROVEN. **Twelve primary classifications** — `schedule_finality_mapping_drift`, `appearance_ledger_completion_authority_defect`, `stored_schedule_row_conflict`, `exact_game_ingestion_gap`, `appearance_ledger_query_defect`, `publication_gate_scope_defect`, `snapshot_state_transition_defect`, `reschedule_or_suspension_identity_defect`, `multiple_contributing_defects`, `incident_condition_no_longer_reproducible`, `no_platform_defect_proven`, `unproven` — each carrying positive supporting evidence, explicit counter-evidence, HIGH/MEDIUM/LOW confidence, and whether the condition persists. **FAILED is reserved for a violation of the AUDIT'S OWN safety contract**; a platform defect the audit discovers is a successful audit and exits 0 as `COMPLETE_ROOT_CAUSE_IDENTIFIED`. The 60-game membership is labelled a CURRENT RECONSTRUCTION because the retained artifacts carry the count and not the list, and the reconstruction is checked against the canonical completeness count rather than overriding it. Exactly one informational next-package category is emitted and it authorizes nothing. Also recorded: the canonical modules the audit reads through are byte-identical to the incident SHA, so no code-behaviour drift explains the incident. No migration was added; the Alembic head remains `c7f1b408d93a`. Daily and postgame remain `shadow`, backfill remains off, publication authority is unchanged, the legacy writer remains production-authoritative, automated write and authoritative modes remain unapproved, and the global dead-letter backlog remains governed at 1,389 and is never reported as zero | Adopted |
...
| Aug 4, 2026 | Activation | **First no-op qualification refused safely; read-only candidate audit built (not executed)** | `feat/noop-qualification-candidate-audit` | Repository change only; run `30862655470` was a governed production dispatch that entered no write phase and mutated nothing. This package performed no dispatch, no production execution, no row mutation, no work-item creation, no Render/Vercel/secret change, no migration, no mode change | Game 824488 failed on `target_work_item_missing`, proving a reconciling GameLog row is not a completed durable work item. Candidate selection moves to a manual-only, read-only, bounded audit over completed work items using the canonical shadow lane per candidate, with three-layer read-only proof and positive-evidence eligibility. Zero candidates is a completed audit. The premature `finality_proven_by_planner` claim is corrected with explicit execution-state fields and a regression fixture for the run. See D-042 |
| Aug 4, 2026 | Reliability | **Read-only postgame publication incident audit built (not executed)** | `feat/postgame-publication-incident-audit` | Repository change only; no workflow dispatch, no rerun of run `30873422601`, no rerun of the failed postgame sync, no production execution, no production database session, no row mutation, no schedule-status change, no appearance-row creation, no marker reset, no snapshot publication or modification, no work-item creation, no Render/Vercel/Supabase/secret change, no migration, no mode change | Run `30873422601` exited 1 with the shadow lane PASS and legacy postgame ingestion complete; the failure was publication qualification. A manual, exact-scope, read-only audit reconstructs the cycle from four separately labelled evidence sources, answers eight questions individually, verifies artifact digests, bounds the MLB source to 20 calls, proves read-only three ways over six fingerprint scopes, and emits one of twelve classifications with positive evidence and counter-evidence. Whether 822867 matches the ledger/planner membership hypothesis is for the audit to prove, not for this package to assert. See D-043 |
...
| 2.5 | August 4, 2026 | Nickolis Kacludis | Recorded D-042: the first production no-op write qualification (run `30862655470`, game 824488) refused with `target_work_item_missing` while every safety property held and no write phase was entered. Recorded the finding that a reconciling GameLog row is not a completed durable game-driven work item, and that operator intuition therefore cannot identify a qualification target. Recorded the manual-only, read-only candidate audit: bounded deterministic discovery over completed work items, the canonical shadow lane per candidate, positive-evidence eligibility, one primary classification under a declared precedence, and three-layer read-only proof via the acquire-only advisory lock, a PostgreSQL read-only transaction with a refused write probe, and before/after content fingerprints across all six reachable tables. Recorded that zero eligible candidates is a completed audit that exits 0, that the audit creates no work item and dispatches nothing, and that the suggested candidate is informational only. Recorded two database guarantees discovered during implementation — the unique work-item game key and the completion-proof constraint — which make two of the requested classifications unreachable while they stand. Recorded the correction of the premature finality claim with explicit execution-state fields and a regression fixture. No migration; automated write and authoritative modes remain unapproved. Recorded the pre-merge review correction of six runtime/evidence blockers: the complete shared plan-governance requirement with a positive eligibility predicate, the required canonical successful shadow status, the corrected work-item-before-shadow execution order proven through the real orchestration, honest read-only probe evidence replacing a false zero-SQL-write claim, the corrected bounded stop reason, and separated selected/evaluated candidate counts. |
| 2.6 | August 4, 2026 | Nickolis Kacludis | Recorded D-043: the read-only postgame publication incident audit for run `30873422601`. Recorded the governing classification that separates a PASS game-driven shadow execution and a successfully completed legacy postgame ingestion from a FAILED appearance-ledger qualification, a FAILED dashboard publication qualification, and a FAILED complete activation cycle — and that a clean reconciliation never proves publication correct, nor a publication failure a broken reconciliation engine. Recorded the immutable incident evidence in full: the 35/35/35 zero-mutation shadow lane, the legacy postgame counts and the five newly processed games, the eight-game preflight mapping with 822867/824324/825095 at `other`, the 133/132 ledger window with 9 mismatches across 9 named MLBAM ids and missing game_pk 822867, snapshot 344 pending against snapshot 343 still serving with five publication-proof reason codes, and the 102/42/60 completeness block. Recorded the ledger-versus-planner membership mechanism explicitly as a hypothesis for the audit to prove or refute rather than as a finding. Recorded the four separately labelled evidence sources and that current state is never presented as incident evidence. Recorded exact-run-id artifact addressing with digest verification where an unexposed digest is UNVERIFIED and a differing digest is FAILED, required versus optional artifacts, and validation of run id, head SHA, cycle kind, slate date, runner exit code and disputed game id from inside the documents. Recorded the six scoped fingerprints, the bounded refused probe, the acquire-only lock, and the finding that correction provenance is columnar rather than a table. Recorded the 8/10/10/20 source-call budget with full call accounting and exhaustion as UNPROVEN. Recorded the twelve classifications with positive evidence, counter-evidence and confidence, that FAILED is reserved for the audit's own safety contract while a discovered platform defect exits 0, that the 60-game set is a current reconstruction checked against the canonical count, and that the single next-package recommendation authorizes nothing. Recorded that the canonical modules are byte-identical to the incident SHA. No migration; modes unchanged; dead-letter backlog still governed at 1,389 and never reported as zero. |
