...
| D-043 | Aug 4, 2026 | **The failed postgame publication cycle is diagnosed by a manual, exact-scope, read-only incident audit rather than by argument; nothing about the incident is repaired here.** Scheduled run `30873422601` (branch `main`, head `9f9f640799af973f0a39cdafb1db83fba473b10c`, cycle `postgame`, slate 2026-08-03) failed while every component reported success: the game-driven shadow lane completed cleanly, legacy postgame ingestion completed its baseball work, publication proof still failed, game 822867 was mapped `other` by the schedule finality authority while the appearance ledger counted it as a completed game requiring appearance rows, dashboard snapshot 344 stayed pending while 343 kept serving, and the completeness report counted 60 unresolved final games. **The mechanism recorded from code is that two membership rules were being read as one**: `services.appearance_ledger` counts a game when ANY stored `scheduled_games` row carries `status_state = final` inside the trailing window, while `services.game_ingestion_planner` plans a game only when EVERY stored row agrees on `final` — and `other` is where the finality authority places cancelled, live/in-progress, abstract-final-without-final-status, and unknown status alike. A game can therefore be inside ledger membership and outside planning scope at the same instant, producing a deficit no lane can close and a correct, permanent withholding. **Whether that is what happened to game 822867 is not asserted here — it is what the audit is built to prove or refute against production evidence.** **Decision: reconstruct the cycle read-only, exactly once, and answer eight explicit questions individually** — the official MLB status of 822867; its stored schedule state; why the preflight produced `other`; why the ledger counted it completed; its exact stored baseball state; what individually explains each reported player appearance mismatch; how each unresolved final game classifies; and why snapshot 344 stayed pending while 343 kept serving, with what sync run 596 recorded. One unanswered question makes the whole audit UNPROVEN, and no question is ever answered by the absence of a failure. **The audit owns nothing it diagnoses**: schedule-finality mapping, appearance-ledger membership, game-ingestion completeness, snapshot publication rules, and reconciliation all remain with their canonical services, which the audit calls and records rather than reimplements; where it reconstructs the set of games behind the completeness count it checks itself against that authority's number and reports `reconciles_with_authority: false` rather than overriding it. **Evidence comes from the incident run itself**, addressed by exact run id and exact name: `game-driven-shadow-30873422601` and `appearance-ledger-audit-30873422601` are required and their absence is UNPROVEN; `game-driven-shadow-handoff-30873422601` is optional because it carries a 7-day retention, and its absence is REPORTED rather than inferred away. A required artifact that is present but unparseable is also UNPROVEN — an empty successful reading and a failed reading must never look the same. **Read-only is proven three ways**, reused unchanged from D-042: the acquire-only public sync advisory lock, a PostgreSQL `SET TRANSACTION READ ONLY` with a bounded `WHERE 1 = 0` write probe that must be refused, and before/after content fingerprints across `scheduled_games`, `game_logs`, `pitchers`, `postgame_processed_games`, `game_ingestion_work_items`, `team_game_pitching_splits`, `completed_game_contexts`, `dashboard_snapshots`, `sync_runs`, and `sync_failures`. Exactly one SQL write statement is issued — the proof itself — and zero durable writes; the two are reported as different fields. **The upstream source is bounded**: at most 8 schedule, 10 exact-game, 10 box-score, and 20 total calls, reserved before issue so the budget refuses rather than overspends, with an exhausted or refusing budget making the audit UNPROVEN rather than shortening it to a verdict. **Twelve named classifications** partition exhaustively into five that name a platform defect — read-only contract violated, schedule-row finality conflict, stored schedule state diverging from source, the finality authority mapping a game out of planning scope, and ledger membership diverging from planner membership — and seven that name correct fail-closed behaviour: missing appearance rows for a final game, missing or incomplete postgame markers, an unresolved work-item backlog, snapshot withholding by the appearance-ledger gate or by slate coverage, and a publication candidate not selected for serving. Results are `COMPLETE_ROOT_CAUSE_IDENTIFIED`, `COMPLETE_NO_PLATFORM_DEFECT_PROVEN`, and `COMPLETE_INCIDENT_NOT_REPRODUCIBLE` at exit 0, `FAILED` at 1, and `UNPROVEN` at 2; FAILED outranks UNPROVEN outranks every COMPLETE result, and an UNPROVEN run never reports a platform defect as proven. **Evidence is scanned and uploaded with 30-day retention before the final gate**, so a failing diagnostic still leaves a reviewable artifact — and the gate still reads the audit's own exit code and fails on 1 and on 2, with no `continue-on-error`. Manual-only, main-only, owner-only, 20-minute bound, `baseballos-sync` concurrency with `cancel-in-progress: false`; permissions are `contents: read` and `actions: read`, the second only so the incident run's artifacts can be downloaded, and no scope is granted write. **Naming a root cause is not repairing one.** This package reruns nothing, dispatches nothing, repairs no game, publishes or modifies no snapshot, resets no marker, creates no work item, and does not widen the MLB source authority. No migration was added; the Alembic head remains `c7f1b408d93a`. Daily and postgame remain `shadow`, backfill remains off, publication authority is unchanged, the legacy writer remains production-authoritative, and automated write and authoritative modes remain unapproved | Adopted |
...
| Aug 4, 2026 | Reliability | **Read-only postgame publication incident audit built (not executed)** | `feat/postgame-publication-incident-audit` | Repository change only; no workflow dispatch, no rerun of run `30873422601`, no rerun of the failed postgame sync, no production execution, no row mutation, no schedule-status change, no appearance-row creation, no marker reset, no snapshot publication or modification, no work-item creation, no Render/Vercel/secret change, no migration, no mode change | Run `30873422601` failed with every component reporting success. The mechanism recorded from code is that appearance-ledger membership (ANY stored row `final`) and planner membership (EVERY stored row `final`) are different tests, so a game can be inside one and outside the other. A manual, exact-scope, read-only audit reconstructs the cycle and answers eight questions individually from production evidence plus the run's own artifacts, bounded to 20 upstream calls, with three-layer read-only proof and twelve named classifications. Whether 822867 matches that mechanism is for the audit to prove, not for this package to assert. See D-043 |
...
| 2.6 | August 4, 2026 | Nickolis Kacludis | Recorded D-043: the read-only postgame publication incident audit for run `30873422601`. Recorded the contradiction the cycle reported — a clean shadow lane, completed legacy postgame baseball work, a failed publication proof, game 822867 mapped `other` while the appearance ledger counted it as a completed game requiring rows, snapshot 344 pending while 343 kept serving, and 60 unresolved final games. Recorded the mechanism read from code: appearance-ledger membership needs ANY stored schedule row to be `final` while planner membership needs EVERY row to agree, and `other` is where the finality authority places cancelled, live, abstract-final-without-final-status, and unknown status alike — so a game can sit inside one membership and outside the other, producing a deficit no lane can close. Recorded explicitly that whether game 822867 matches that mechanism is for the audit to prove against production evidence, not for the implementation to assert. Recorded the eight questions the audit answers individually and that one unanswered question makes the whole audit UNPROVEN. Recorded that the audit owns none of the authorities it diagnoses and checks its own reconstruction against the completeness authority's count rather than overriding it. Recorded the exact-run-id artifact contract with two required artifacts, one optional artifact whose absence is reported rather than inferred away, and unparseable required evidence treated as UNPROVEN. Recorded the reused three-layer read-only proof, the single bounded refused write probe against zero durable writes, the bounded MLB source-call budget reserved before issue with exhaustion as UNPROVEN, the twelve named classifications partitioned exhaustively into five platform-defect and seven fail-closed conditions, and the five results with FAILED outranking UNPROVEN outranking every COMPLETE result. Recorded that evidence is scanned and uploaded before the final gate while the gate still fails on the audit's own exit codes 1 and 2. Recorded that this package executed nothing: no dispatch, no rerun, no repair, no snapshot publication, no marker reset, no work-item creation, no migration, Alembic head unchanged at `c7f1b408d93a`, modes unchanged, and automated write and authoritative modes still unapproved. |
