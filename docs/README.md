...
As of August 24, 2026, with audited `origin/main` at `6360a4b5`:
...
- **TODAY-05 Rotation Transfer Context is complete.** Every eligible Tonight side carries exact frozen short-start and bullpen-innings facts through one shared trusted-snapshot resolution; missing evidence and side-local failures remain intact, with no forecast or browser-request growth.
- **Since Yesterday is compact by default.** Every detailed team and its canonical explanation remain reachable through one-at-a-time expansion; backend What Changed semantics and the request graph are unchanged.
- **TODAY-06 Tonight Slate Scanability is the single active objective.** It is bounded to compacting every existing game by default and revealing one unchanged full game detail on request; it adds no facts, ranking, prediction, backend semantic change, or browser request.
