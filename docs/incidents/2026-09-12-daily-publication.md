...
- The public response's cache serves the same database publication identity. Cache eviction cannot create a missing trusted publication. Static distribution was skipped in the failed fallback. The last inspected generated-route commit identifies snapshot 2604 / September 9. A live Twitterbot request to `https://baseballos.app/team/CWS` returned HTTP 200 with description and Open Graph description still covering September 9.

## Narrow remediation and validation

Branch `fix/daily-publication-team-authority` starts at the exact production commit above in an isolated checkout. It restricts both team-directory queries and the public teams endpoint to the existing canonical MLB club registry while requiring actual active-pitcher evidence. It does not fill missing clubs, mutate affiliate rows, change classifiers, weaken the exact-30 proof, or advance integration/schema authority. Existing batch/operations fixtures now use actual MLB IDs. The new regression reproduces the 34-team production population, checks the endpoint and directory against the same 30-club authority, and proves that missing MLB evidence still blocks publication.
