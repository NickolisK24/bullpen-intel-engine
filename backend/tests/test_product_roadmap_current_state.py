...
def test_roadmap_declares_version_5_7():
    text = _roadmap_text()
...
def test_decision_ledger_is_contiguous_through_d058():
    ...
    # 5.6 is main's TODAY-05 closeout and adds no ID; 5.7 adds D-058. The
...
def test_revision_history_preserves_the_version_5_6_entry():
    """5.6 is main's TODAY-05 closeout; 5.7 is additive to it."""
    text = _roadmap_text()
    rows = [
        line for line in text.splitlines()
        if line.startswith('| 5.6 | August 24, 2026 |')
    ]
    assert len(rows) == 1, 'exactly one historical Version 5.6 revision-history row'
    for claimed in (
...
def test_revision_history_preserves_the_version_5_5_entry():
    """5.5 is main's TODAY-04 closeout."""
    text = _roadmap_text()
    rows = [
        line for line in text.splitlines()
...
def test_revision_history_preserves_the_version_5_3_entry():
    ...
    assert len(rows) == 1, 'exactly one historical Version 5.3 revision-history row'
    for claimed in (
        '`origin/main` `326e4da2`',
        'closed TODAY-02',
        'TODAY-03 Pregame Bullpen Signals',
        'already-published public Team State listing',
        'No durable decision was added or changed',
    ):
        assert claimed in rows[0], claimed
...
def test_revision_history_preserves_the_version_5_2_entry():
    """5.2 is main's TODAY-01 closeout."""
    text = _roadmap_text()
    rows = [
        line for line in text.splitlines()
