...
def test_history_api_validates_season_and_team(client, app):
    ...
    assert response.get_json()['contract'] == 'team_state_history_v2'
...
def test_v2_preserves_hist_01_row_fields_additively(app):
    artifact = _publish(date(2026, 7, 23))

    payload = build_team_state_history('TST', season=2026)
    row = payload['rows'][0]

    assert payload['contract'] == 'team_state_history_v2'
    assert {
