...
def test_direct_alembic_cannot_bypass_authority(tmp_path, mode):
    database = tmp_path / 'authority.db'
    with sqlite3.connect(database) as conn:
        conn.execute('CREATE TABLE preserved_test_data (id integer PRIMARY KEY)')
...
def test_head_inspection_emits_no_mutation(tmp_path, monkeypatch):
    ...
    with sqlite3.connect(database) as connection:
        connection.execute('CREATE TABLE preserved_test_data (id integer PRIMARY KEY)')
    before = database.read_bytes()
...
def test_maintenance_promotion_is_bounded():
