...
@pytest.fixture
def admission_evidence(app):
    """Represent migrated evidence domains without importing integration models."""
    from sqlalchemy import Boolean, Column, Integer, MetaData, Table, Text
    metadata = MetaData()
    membership = Table('roster_membership_intervals', metadata,
                       Column('id', Integer, primary_key=True),
                       Column('correction_reason', Text))
    versions = Table('final_game_versions', metadata,
                     Column('id', Integer, primary_key=True),
                     Column('is_current', Boolean), Column('fact_fingerprint', Text))
    with app.app_context():
        metadata.create_all(db.engine)
        yield membership, versions
        db.session.remove()
        metadata.drop_all(db.engine)


def test_continuous_admission_survives_restart_and_reopens_on_dependencies(app, monkeypatch, admission_evidence):
...
def test_continuous_admission_crash_has_only_durable_rejection_receipts(
    app, monkeypatch, committed_before_crash, admission_evidence,
):
    from services import continuous_production_publication as publication
    represented = date(2026, 9, 13)
    builds = []
...
def test_admission_signature_tracks_corrections_finality_authority_and_ignores_polls(app, admission_evidence):
    ...
    with app.app_context():
        ...
        membership, versions = admission_evidence
        before = signature()
        db.session.execute(membership.insert().values(id=1, correction_reason='original'))
        db.session.commit()
        assert signature() != before
        before = signature()
        db.session.execute(membership.update().values(correction_reason='corrected'))
        db.session.commit()
        assert signature() != before
        before = signature()
        db.session.execute(versions.insert().values(id=1, is_current=False, fact_fingerprint='old'))
        db.session.commit()
        assert signature() == before
        db.session.execute(versions.update().values(is_current=True))
        db.session.commit()
        assert signature() != before


def test_missing_dependency_domain_cannot_authorize_admission_suppression(app):
    from services.continuous_publication_admission import dependency_signature
    with app.app_context():
        assert dependency_signature(current_publication_id=41,
                                    represented_date=date(2026, 9, 13)) is None
