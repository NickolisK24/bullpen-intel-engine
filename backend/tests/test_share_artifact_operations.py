...
def test_terminal_states_are_accounted(app, monkeypatch):
    _seed_teams(CANON_TEAMS)
    _install_snapshot(monkeypatch)
    _generate(108)                                   # generated (real artifact)
    _generate(109); _generate(109)                   # reused (second attempt)
    _audit(110, ShareArtifactGenerationAudit.OUTCOME_REFUSED, blocking=('insufficient_trust',))
    _audit(111, ShareArtifactGenerationAudit.OUTCOME_FAILED_CLOSED, failure_code='publication_error')
    # A 5th canonical team with no attempt -> missing.
    db.session.add(Pitcher(mlb_id=955, full_name='Arm 112', team_id=112, active=True))
    db.session.flush()

    overview = build_coverage_overview()
    rows = _overview_by_team(overview)
    assert rows[108]['state'] == COVERAGE_GENERATED and rows[108]['public_id']
    assert rows[109]['state'] == COVERAGE_REUSED
    assert rows[110]['state'] == COVERAGE_REFUSED and rows[110]['reason_code'] == 'insufficient_trust'
    assert rows[111]['state'] == COVERAGE_FAILED and rows[111]['failure_code'] == 'publication_error'
    assert rows[112]['state'] == COVERAGE_MISSING
...
def test_most_recent_attempt_controls_coverage_without_double_count(app, monkeypatch):
    _seed_teams((108,))
    _install_snapshot(monkeypatch)
    _audit(108, ShareArtifactGenerationAudit.OUTCOME_REFUSED, blocking=('insufficient_trust',),
           created_at=datetime(2026, 7, 20, 10, 0, 0))
    _audit(108, ShareArtifactGenerationAudit.OUTCOME_PUBLISHED, public_id='pub-108',
           created_at=datetime(2026, 7, 20, 12, 0, 0))  # later
    overview = build_coverage_overview()
    rows = _overview_by_team(overview)
    assert rows[108]['state'] == COVERAGE_GENERATED   # latest terminal wins
    ...
    audits = list_operational_audits(team_id=108)
...
def test_other_snapshot_artifact_does_not_satisfy_coverage(app, monkeypatch):
    ...
    _generate(108)  # published under snapshot 7001 / 2026-07-19
    ...
    rows = _overview_by_team(overview)
    assert rows[108]['state'] == COVERAGE_MISSING     # 7001 artifact does not count for 7002
    assert overview['missing_team_count'] == 1
...
def test_accounting_invariants_hold(app, monkeypatch):
    ...
    _install_snapshot(monkeypatch)
    _generate(108)
    _audit(109, ShareArtifactGenerationAudit.OUTCOME_REFUSED, blocking=('data_limited',))
    overview = build_coverage_overview()
...
def test_impossible_accounting_fails_closed(app, monkeypatch):
    ...
    _audit(108, ShareArtifactGenerationAudit.OUTCOME_REFUSED)
...
def test_deterministic_team_ordering(app, monkeypatch):
    _seed_teams((110, 108, 109))
    _install_snapshot(monkeypatch)
    overview = build_coverage_overview()
    assert [t['team_id'] for t in overview['teams']] == [108, 109, 110]
...
def test_status_degraded_on_failure(app, monkeypatch):
    ...
    _install_snapshot(monkeypatch)
    _generate(108)
    _audit(109, ShareArtifactGenerationAudit.OUTCOME_FAILED_CLOSED, failure_code='publication_error')
...
def test_status_incomplete_on_missing(app, monkeypatch):
    _seed_teams((108, 109))
    _install_snapshot(monkeypatch)
    _generate(108)  # 109 has no attempt -> missing
...
def test_integrity_verified(app, monkeypatch):
    ...
    assert rows[108]['integrity_state'] == INTEGRITY_VERIFIED
...
def test_integrity_mismatch(app, monkeypatch):
    _seed_teams((108,))
    _install_snapshot(monkeypatch)
    artifact = _generate(108).artifact
    ...
    overview = build_coverage_overview()
    rows = _overview_by_team(overview)
    assert rows[108]['integrity_state'] == INTEGRITY_MISMATCH
...
def test_integrity_error_without_mutation(app, monkeypatch):
    ...
    overview = build_coverage_overview()
    rows = _overview_by_team(overview)
    assert rows[108]['integrity_state'] == INTEGRITY_ERROR
...
def test_recent_artifacts_newest_first_and_safe(app, monkeypatch):
    ...
    # 24 — newest first (team 109 generated last).
    assert payload['artifacts'][0]['team_id'] == 109
...
def test_recent_artifacts_limit_offset_and_filter(app, monkeypatch):
    ...
    # team filter.
    only101 = list_operational_artifacts(team_id=108)
    assert {a['team_id'] for a in only101['artifacts']} == {108}
...
def test_recent_audits_distinct_outcomes_and_codes(app, monkeypatch):
    _seed_teams((108, 109, 110))
    _install_snapshot(monkeypatch)
    _generate(108)
    _audit(109, ShareArtifactGenerationAudit.OUTCOME_REFUSED, blocking=('insufficient_trust',))
    _audit(110, ShareArtifactGenerationAudit.OUTCOME_FAILED_CLOSED, failure_code='publication_error')
    payload = list_operational_audits(limit=10)
    by_team = {a['team_id']: a for a in payload['audits']}
    # 29 — distinct outcomes.
    assert by_team[108]['outcome'] == 'published'
    assert by_team[109]['outcome'] == 'refused'
    assert by_team[110]['outcome'] == 'failed_closed'
