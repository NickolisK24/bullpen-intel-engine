...
def ensure_obligation(change, *, row=None, sync_run_id=None, commit=True):
    ...
    job = SyncJob(
        ...
        scope_type=sync_jobs.JobScopeType.GAME.value,
        scope_key=scope_key,
        product_date=_product_date(row, now),
        payload_schema_version=SUPPORTED_SCHEMA_VERSION,
        dedupe_key=sync_jobs.legacy_dedupe_key(
            JOB_NAME,
            scope_key,
            _product_date(row, now),
        ),
    ...
    *, reference_date, correction_days, limit, sync_run_id=None,
    ...
    for item in items:
        ...
        job = SyncJob(
            ...
            dedupe_key=sync_jobs.legacy_dedupe_key(
                JOB_NAME,
                f'game:{item.mlb_game_pk}{daily_suffix}',
                item.represented_date,
            ),
            status=sync_jobs.STATUS_PENDING,
            attempts=0,
            max_attempts=MAX_ATTEMPTS,
