...
def qualifying_appearances(team_id, pitcher_ids, *, season, through_date=None):
    """Official completed relief appearances made FOR this team.

    Excluded by construction: starts, appearances owned by another
    organization, unresolved appearance-team authority, and non-regular-season
    game types.
    """
    if not pitcher_ids:
        return []
    ...
    query = (
        ...
        .filter(
            ...
            GameLog.game_date <= date(season, 12, 31),
        )
    )
    ...
    return [
        log
        for log in query.order_by(GameLog.game_date.asc(), GameLog.id.asc()).all()
        if _is_relief(log)
    ]
...
def _is_relief(log):
    try:
        return games_started_state(getattr(log, 'games_started', None)) == RELIEF
    except Exception:
        # A malformed start flag never counts as a qualifying relief outing.
        return False
...
def build_components(logs) -> AppearanceComponents:
    ...
    return AppearanceComponents(
        ...
        outs=sum(_int(getattr(r, 'innings_pitched_outs', 0)) for r in rows),
        earned_runs=sum(_int(getattr(r, 'earned_runs', 0)) for r in rows),
        runs=sum(_int(getattr(r, 'runs_allowed', 0)) for r in rows),
        hits=sum(_int(getattr(r, 'hits_allowed', 0)) for r in rows),
        walks=sum(_int(getattr(r, 'walks', 0)) for r in rows),
        strikeouts=sum(_int(getattr(r, 'strikeouts', 0)) for r in rows),
        home_runs=sum(_int(getattr(r, 'home_runs_allowed', 0)) for r in rows),
        batters_faced=sum(_int(getattr(r, 'batters_faced', 0)) for r in rows),
    )
        ...
        'evidence': [
            {
                ...
                'outs': _int(getattr(log, 'innings_pitched_outs', 0)),
                'earned_runs': _int(getattr(log, 'earned_runs', 0)),
            }
            for log in logs
        ],
        'official_record': {
